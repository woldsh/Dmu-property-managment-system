...
export default function StoreFixedSidebar() {
    const pathname = usePathname();
    const basePath = '/procurement-management/store/fixed-material';
    ...
    const menuItems = [
        ...
        { label: 'Register Asset', href: `${basePath}/add-items`, icon: FaPlusCircle },
            ...
            `}>
                <div className="w-72 flex flex-col h-full flex-shrink-0">
                    ...
                    <div className="p-6 border-b border-slate-800/50 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 text-white">
                            <FaStore className="text-xl" />
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-white tracking-tight">Store Keeper</h2>
                            <p className="text-xs font-medium text-emerald-400 uppercase tracking-wider">Fixed Assets</p>
                        </div>
                    </div>
                                ...
                                <div key={index}>
                                    <Link
                                        ...
                                        className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group relative overflow-hidden ${isActive
                                            ? 'bg-gradient-to-r from-emerald-600/20 to-teal-600/10 text-emerald-400 font-semibold'
                                    ...
                                    >
                                        ...
                                        <Icon className={`text-lg transition-colors ${isActive ? 'text-emerald-400' : 'text-slate-500 group-hover:text-emerald-400'}`} />
                                    ...
                                    {item.subItems && (
                                        <div className="ml-12 mt-1 space-y-1 border-l border-slate-800 pl-3">
                                            {item.subItems.map(sub => (
                                                <div key={sub} className="text-xs text-slate-500 hover:text-emerald-400 py-1 cursor-pointer transition-colors block">
                                                    {sub}
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div className="p-4 border-t border-slate-800/50 bg-slate-950/50 backdrop-blur-sm">
                        <div className="flex items-center gap-3 rounded-lg p-2 hover:bg-slate-900 transition-colors cursor-pointer">
                            ...
                            <div className="overflow-hidden">
                                ...
                                <p className="text-xs text-slate-500 truncate">store@fixed</p>
                            </div>
                        </div>
                    </div>
                </div>
