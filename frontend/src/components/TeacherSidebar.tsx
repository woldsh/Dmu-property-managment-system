...
export default function TeacherSidebar() {
    ...
    const basePath = '/academic-staff/teachers';
    ...
    const menuItems = [
        ...
        { label: 'Request to Dept Head', href: `${basePath}/request-material`, icon: FaUserTie },
            ...
            `}>
                <div className="w-72 flex flex-col h-full flex-shrink-0">
                    ...
                    <div className="p-6 border-b border-slate-800/50 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center shadow-lg shadow-purple-500/20 text-white">
                            <FaChalkboardTeacher className="text-xl" />
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-white tracking-tight">Teacher</h2>
                            <p className="text-xs font-medium text-purple-400 uppercase tracking-wider">Academic Staff</p>
                        </div>
                    </div>
                                ...
                                <div key={index}>
                                    <Link
                                        ...
                                        className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group relative overflow-hidden ${isActive
                                            ? 'bg-gradient-to-r from-purple-600/20 to-pink-600/10 text-purple-400 font-semibold'
                                    ...
                                    >
                                        ...
                                        <Icon className={`text-lg transition-colors ${isActive ? 'text-purple-400' : 'text-slate-500 group-hover:text-purple-400'}`} />
                                    ...
                                    {item.subItems && (
                                        <div className="ml-12 mt-1 space-y-1 border-l border-slate-800 pl-3">
                                            {item.subItems.map(sub => (
                                                <div key={sub} className="text-xs text-slate-500 hover:text-purple-400 py-1 cursor-pointer transition-colors block">
                                                    {sub}
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div className="p-4 border-t border-slate-800/50 bg-slate-950/50 backdrop-blur-sm">
                        <div className="flex items-center gap-3 rounded-lg p-2 hover:bg-slate-900 transition-colors cursor-pointer">
                            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center border border-slate-700">
                                <span className="text-xs font-bold text-slate-400">TC</span>
                            </div>
                            <div className="overflow-hidden">
                                <p className="text-sm font-medium text-white truncate">Teacher</p>
                                <p className="text-xs text-slate-500 truncate">teacher@academic</p>
                            </div>
                        </div>
                    </div>
                </div>
