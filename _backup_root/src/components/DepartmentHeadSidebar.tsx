...
export default function DepartmentHeadSidebar() {
    ...
    const basePath = '/academic-staff/department-head';
    ...
    const menuItems = [
        ...
        { label: 'Approve & Send (AC)', href: `${basePath}/approve-decisions`, icon: FaCheckCircle },
        { label: 'Join Meeting', href: `${basePath}/join-meeting`, icon: FaVideo },
        { label: 'Request to AC', href: `${basePath}/request-material`, icon: FaUserTie },
            ...
            `}>
                <div className="w-72 flex flex-col h-full flex-shrink-0">
                    ...
                    <div className="p-6 border-b border-slate-800/50 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center shadow-lg shadow-orange-500/20 text-white">
                            <FaBuilding className="text-xl" />
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-white tracking-tight">Dept Head</h2>
                            <p className="text-xs font-medium text-orange-400 uppercase tracking-wider">Academic Staff</p>
                        </div>
                    </div>
                                ...
                                <div key={index}>
                                    <Link
                                        ...
                                        className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group relative overflow-hidden ${isActive
                                            ? 'bg-gradient-to-r from-orange-600/20 to-red-600/10 text-orange-400 font-semibold'
                                            : 'hover:bg-slate-900 hover:text-white'
                                            }`}
                                    >
                                        {isActive && (
                                            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-orange-500 rounded-r-full shadow-[0_0_8px_rgba(249,115,22,0.5)]" />
                                        )}
                                        <Icon className={`text-lg transition-colors ${isActive ? 'text-orange-400' : 'text-slate-500 group-hover:text-orange-400'}`} />
                                    ...
                                    {item.subItems && (
                                        <div className="ml-12 mt-1 space-y-1 border-l border-slate-800 pl-3">
                                            {item.subItems.map(sub => (
                                                <div key={sub} className="text-xs text-slate-500 hover:text-orange-400 py-1 cursor-pointer transition-colors block">
                                                    {sub}
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div className="p-4 border-t border-slate-800/50 bg-slate-950/50 backdrop-blur-sm">
                        <div className="flex items-center gap-3 rounded-lg p-2 hover:bg-slate-900 transition-colors cursor-pointer">
                            ...
                            <div className="overflow-hidden">
                                <p className="text-sm font-medium text-white truncate">Dept Head</p>
                                <p className="text-xs text-slate-500 truncate">head@dept</p>
                            </div>
                        </div>
                    </div>
                </div>
