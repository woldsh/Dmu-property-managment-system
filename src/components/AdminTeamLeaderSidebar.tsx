    ...
    return (
        <>
            ...
            >
                ...
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,_rgba(16,185,129,0.05)_0%,_transparent_50%),radial-gradient(circle_at_80%_80%,_rgba(20,184,166,0.05)_0%,_transparent_50%)]" />
                    ...
                    <div className="relative p-7 pb-6">
                        ...
                        <div className="relative flex items-center gap-4">
                            <div className="relative group/logo cursor-pointer">
                                <div className="absolute -inset-2 bg-gradient-to-r from-emerald-600 via-teal-500 to-cyan-400 rounded-2xl blur-xl opacity-20 group-hover/logo:opacity-50 transition-all duration-700 scale-90 group-hover/logo:scale-110" />
                                <div className="relative w-14 h-14 rounded-2xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-slate-100 flex items-center justify-center transition-all duration-500 group-hover/logo:shadow-emerald-500/20 group-hover/logo:-translate-y-1">
                                    <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/10 to-transparent rounded-2xl opacity-0 group-hover/logo:opacity-100 transition-opacity" />
                                    <FaUserShield className="text-2xl text-emerald-600 drop-shadow-sm" />
                                </div>
                            </div>
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <p className="text-[10px] font-black text-emerald-600/60 uppercase tracking-[0.3em] font-mono">{t('administration_label')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            return (
                                <Link key={index} href={item.href} onClick={handleLinkClick} style={{ animationDelay: `${index * 40}ms` }}
                                    className={`menu-item-premium relative flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-500 group overflow-hidden
                                        ${isActive ? 'bg-white shadow-[0_10px_25px_-5px_rgba(16,185,129,0.12)] border border-emerald-100/50 text-emerald-600' : 'text-slate-500 hover:text-slate-900'}`}>

                                    <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 rounded-r-full transition-all duration-500
                                        ${isActive ? 'bg-emerald-600 shadow-[2px_0_10px_rgba(16,185,129,0.4)] opacity-100' : 'h-0 opacity-0 group-hover:h-3 group-hover:bg-slate-200 group-hover:opacity-100'}`} />

                                    <div className={`relative flex items-center justify-center w-11 h-11 rounded-xl transition-all duration-500
                                        ${isActive ? 'bg-emerald-50 shadow-inner' : 'bg-slate-50/50 group-hover:bg-emerald-50 group-hover:scale-110 group-hover:rotate-3'}`}>
                                        <Icon className={`text-xl transition-all duration-500 ${isActive ? 'text-emerald-600 scale-110' : 'text-slate-400 group-hover:text-emerald-500'}`} />
                                    </div>
                                    ...
                                    {item.label === t('view_requests') && requestCount > 0 && (
                                        <div className="absolute right-12 top-1/2 -translate-y-1/2 flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-emerald-600 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                                            <span className="text-[10px] font-black text-white">{requestCount}</span>
                                        </div>
                                    )}

                                    <div className={`transition-all duration-500 transform ${isActive ? 'rotate-90 text-emerald-600' : 'opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0'}`}>
