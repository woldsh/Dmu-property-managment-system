                ...
                <div className="relative w-80 flex flex-col h-full flex-shrink-0 border-r border-teal-500/10">
                    <div className="relative p-6 border-b border-white/5">
                        ...
                        <div className="relative flex items-center gap-4">
                            <div className="relative group">
                                <div className="absolute -inset-2 bg-gradient-to-r from-teal-500 via-emerald-500 to-cyan-500 rounded-2xl blur-lg opacity-40 group-hover:opacity-70 animate-pulse transition-opacity duration-500" />
                                <div className="relative w-12 h-12 rounded-xl bg-gradient-to-br from-teal-500 via-emerald-500 to-cyan-500 flex items-center justify-center shadow-2xl">
                                    <FaDribbble className="text-xl text-white drop-shadow-lg" />
                                </div>
                            </div>
                            <div>
                                <h2 className="text-lg font-black text-white tracking-tight">{t('sports_leader')}</h2>
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-teal-400 animate-pulse shadow-lg shadow-teal-400/50" />
                                    <p className="text-xs font-bold text-teal-400/80 uppercase tracking-[0.2em]">{t('sports_service')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
                        <style jsx global>{`
                            ...
                            .custom-scrollbar::-webkit-scrollbar-thumb { background: linear-gradient(to bottom, #14b8a6, #10b981); border-radius: 10px; }
                            ...
                            return (
                                <Link key={index} href={item.href} onClick={handleLinkClick} style={{ animationDelay: `${index * 30}ms` }}
                                    className={`menu-item relative flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group overflow-hidden
                                        ${isActive ? 'bg-gradient-to-r from-teal-500/20 via-emerald-500/15 to-transparent text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>

                                    <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 rounded-r-full transition-all duration-300
                                        ${isActive ? 'h-8 bg-gradient-to-b from-teal-400 via-emerald-500 to-cyan-500 shadow-lg shadow-teal-500/50' : 'h-0'}`} />

                                    <div className={`relative flex items-center justify-center w-10 h-10 rounded-xl transition-all duration-300
                                        ${isActive ? 'bg-gradient-to-br from-teal-500/30 to-emerald-500/20 shadow-lg shadow-teal-500/20' : 'bg-white/5 group-hover:bg-white/10 group-hover:scale-110'}`}>
                    ...
                    <div className="relative p-4 border-t border-white/5">
                        <div className="relative flex items-center gap-3 p-3 rounded-2xl bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 cursor-pointer group">
                            <div className="relative w-10 h-10 rounded-full bg-gradient-to-br from-teal-500 via-emerald-500 to-cyan-500 flex items-center justify-center">
                                <span className="text-sm font-black text-white">SL</span>
                            </div>
                            <div className="flex-1 overflow-hidden">
                                <p className="text-sm font-bold text-white truncate">{t('sports_leader')}</p>
                                <p className="text-xs text-teal-400/60 truncate italic font-medium tracking-wide">{t('student_service')}</p>
                            </div>
                        </div>
                    </div>
                </div>
